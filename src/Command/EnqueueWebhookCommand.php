...
final class EnqueueWebhookCommand extends Command
{
    use LockableTrait;

    private const CHANNEL_ID_ARGUMENT_CODE = 'channel-id';

    public const ENQUEUE_WEBHOOK_COMMAND_STOPWATCH_NAME = 'enqueue-webhook-command';
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addArgument(self::CHANNEL_ID_ARGUMENT_CODE, InputArgument::OPTIONAL, 'The identifier id of the channel which list webhook should be enqueue.')
            ->addOption(self::ALL_OPTION_CODE, 'a', InputOption::VALUE_NONE, 'If set, the command will enqueue lists webhook for all the channels.')
        ;
    }
    ...
    protected function interact(InputInterface $input, OutputInterface $output): void
    {
        ...
        $this->io->title('Enqueue Webhook Command Interactive Wizard');
        $this->io->text([
            ...
            '',
            'Now we\'ll ask you for the value of the channel webhook list id to enqueue.',
        ]);
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $stopwatch->start(self::ENQUEUE_WEBHOOK_COMMAND_STOPWATCH_NAME);
        ...
        if (count($channelsToExport) === 0) {
            $this->io->writeln('No channels founded for which enqueue the webhook list.');
        ...
        $event = $stopwatch->stop(self::ENQUEUE_WEBHOOK_COMMAND_STOPWATCH_NAME);
    ...
    {
        return <<<'HELP'
            The <info>%command.name%</info> command enqueue all list webhooks to export them on ActiveCampaign:

              <info>php %command.full_name%</info> <comment>channel-id</comment>

            By default the command enqueue only one list webhook from channel. To enqueue all channels,
